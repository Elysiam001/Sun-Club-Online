...
function renderTable(requests) {
    ...
    // 1. Hiển thị các yêu cầu đang chờ (pending)
    const pending = requests.filter(r => r.status === 'pending');
    if (pending.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:30px;">Chưa có yêu cầu nào mới.</td></tr>';
    } else {
        tbody.innerHTML = pending.map(req => `
            <tr>
                ...
                <td style="font-size:12px; max-width:250px;">${req.details.replace(/\n/g, '<br>')}</td>
                <td>
    ...
    // 2. Hiển thị lịch sử đã xử lý (approved/rejected)
    const processed = requests.filter(r => r.status !== 'pending').reverse(); // Đảo ngược để cái mới nhất lên đầu
    if (processed.length === 0) {
        historyBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:30px;">Chưa có lịch sử giao dịch.</td></tr>';
    } else {
        historyBody.innerHTML = processed.map(req => `
            <tr>
                <td>${req.time}</td>
                <td style="color:#fff;">${req.username}</td>
                <td><span class="badge ${req.type}">${req.type === 'deposit' ? 'NẠP TIỀN' : 'RÚT TIỀN'}</span></td>
                <td>${parseInt(req.amount).toLocaleString()} VNĐ</td>
                <td><span class="status-${req.status}" style="color: ${req.status === 'approved' ? '#00ff41' : '#ff3333'}">${req.status === 'approved' ? 'THÀNH CÔNG' : 'ĐÃ HỦY'}</span></td>
                <td style="font-size:12px; color:#aaa; font-family:monospace;">${req.details.replace(/\n/g, '<br>')}</td>
            </tr>
        `).join('');
    }
}
